...
import './pengajuan-report-du.css';
...
const PengajuanReportDU: Component = () => {
  ...
  const handleRowClick = (event: { data: any; node: any; }) => {
    const { data, node } = event;
    setSelectedRow(() => ({ id: node.data.id, status: data.status }));
...
export default PengajuanReportDU;